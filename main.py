...
with open(filename) as f:
    ...
    print(f'1) Total word count: {word_count}')
    print(f'2) Unique words: {len(words_seen)}')
    print(f'3) Sentences: {sentence_count}')
    print(f'B1) Average sentence length (in words): {round(word_count / sentence_count, 2)}')
    print(f'B2) Common phrases: ')
    print(*(phrase for phrase in most_common_phrases if phrase[1] >= 3))
    print(f'B3) Word Frequencies: ')
